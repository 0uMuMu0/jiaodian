...
def inference(images, desc_vector):
    ...
    with tf.variable_scope('fc5') as scope:
        weights = _variable_with_weight_decay('weights', shape=[600, 1024],
                                              stddev=0.04, wd=0.004)
        biases = _variable_on_cpu('biases', [1024], tf.constant_initializer(0.1))

        fc5_input = tf.concat(1, [local4, desc_vector])
        fc5 = tf.nn.relu(tf.matmul(fc5_input, weights) + biases, name=scope.name)
        _activation_summary(fc5)

    # fc6
    with tf.variable_scope('fc6') as scope:
        weights = _variable_with_weight_decay('weights', shape=[1024, 1],
                                              stddev=0.04, wd=0.004)
...
def loss(logits, labels):
    ...
    cross_entropy = tf.nn.softmax_cross_entropy_with_logits(logits,
                                                            onehot_labels,
                                                            name='xentropy')
    loss_mean = tf.reduce_mean(cross_entropy, name='xentropy_mean')
